    ...
    public class AudioPlayer : BaseAudioFeedback
    {
        ...
        public AudioSource Play()
        {
            ...
            if (soundFile != null)
            {
                ...
                if (loopSound)
                {
                    source = am.PlaySoundLoopSpecialInternal(soundFile, sTransform, spatialSound, priority, delay, ignoreTimeScale);
                }
                else source = am.PlaySoundInternal(soundFile, sTransform, priority, pitchShift, delay);
            }
            else
            {
                if (loopSound)
                {
                    source = am.PlaySoundLoopSpecialInternal(sound, sTransform, spatialSound, priority, delay, ignoreTimeScale);
                }
                else source = am.PlaySoundSpecialInternal(sound, sTransform, priority, pitchShift, delay, ignoreTimeScale);
            }
