...
namespace JSAM
{
    ...
    public class AudioManagerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (showSoundLibrary)
            {
                ...
                for (int i = 0; i < audioFiles.arraySize; i++)
                {
                    ...
                    if (clickArea.Contains(Event.current.mousePosition) && Event.current.type == EventType.ContextClick)
                    {
                        ...
                        myScript.DebugLog("Copied " + sName.tooltip + " to clipboard!");
                        menu.ShowAsContext();
                    }
                    EditorGUILayout.EndHorizontal();
                }
            ...
            if (showMusicLibrary)
            {
                ...
                for (int i = 0; i < audioFiles.arraySize; i++)
                {
                    ...
                    if (clickArea.Contains(Event.current.mousePosition) && Event.current.type == EventType.ContextClick)
                    {
                        ...
                        menu.AddItem(new GUIContent("Copy \"" + mName.tooltip + "\" to Clipboard"), false, CopyToClipboard, mName.tooltip);
                        myScript.DebugLog("Copied " + mName.tooltip + " to clipboard!");
                        menu.ShowAsContext();
                    }
                    EditorGUILayout.EndHorizontal();
                }
            ...
            if (myScript.GetMasterVolumeInternal() == 0) EditorGUILayout.HelpBox("Note: Master Volume is MUTED!", MessageType.Info);
            if (myScript.GetSoundVolumeInternal() == 0) EditorGUILayout.HelpBox("Note: Sound is MUTED!", MessageType.Info);
            if (myScript.GetMusicVolumeInternal() == 0) EditorGUILayout.HelpBox("Note: Music is MUTED!", MessageType.Info);
