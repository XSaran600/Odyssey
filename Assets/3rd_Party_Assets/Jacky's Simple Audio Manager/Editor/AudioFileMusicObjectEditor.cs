    ...
    public class AudioFileMusicObjectEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (myScript.GetFile() == null)
            {
                ...
                "fadeMode", "clampBetweenLoopPoints", "spatialize", "ignoreTimeScale" });
            }
            else
            {
                propertiesToExclude.AddRange(new List<string>() { "m_Script", "useLibrary", "files" });
            ...
            if (myScript.GetFile() != null)
            {
                ...
                using (new EditorGUI.DisabledScope(myScript.loopMode != LoopMode.LoopWithLoopPoints))
                {
                    ...
                    showLoopPointTool = EditorGUILayout.Foldout(showLoopPointTool, "Loop Point Tools", boldFoldout);
                    if (showLoopPointTool)
                    {
                        EditorGUILayout.Space();

                        EditorGUILayout.LabelField("Customize where music will loop between", EditorStyles.boldLabel);
                        EditorGUILayout.HelpBox("Note: Loops may not appear to be seamless in the inspector but rest assured, they will be seamless in-game!", MessageType.Info);
                        ...
                        DrawPlaybackTool(music, myScript.loopPointInputMode == AudioFileMusicObject.LoopPointTool.BPMInput);
        ...
        public void DrawPlaybackTool(AudioClip music, bool showBeats)
        {
            ...
            if (!showBeats)
            {
                EditorGUILayout.LabelField(TimeToString((float)helperSource.timeSamples / music.frequency) + " / " + (TimeToString(music.length)));
            }
            else
            {
                AudioFileMusicObject myScript = (AudioFileMusicObject)target;
                EditorGUILayout.LabelField(new GUIContent(string.Format("{0:0}", helperSource.time / (60f / myScript.bpm)) + " / " + music.length / (60f / myScript.bpm), "The playback time in beats"));
            }
            EditorGUILayout.EndHorizontal();
