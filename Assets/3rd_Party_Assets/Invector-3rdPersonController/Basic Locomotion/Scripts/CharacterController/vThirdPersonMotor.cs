...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : vCharacter
    {
        ...
        [Tooltip("How long character actual jumps (this determines cooldown before you can do other actions like roll into jump)")]
        public float finalJumpTimer = 0.3f;
        internal float jumpCounter;
        internal float finalJumpCounter;
        [Tooltip("Add Extra jump height, if you want to jump only with Root Motion leave the value with 0.")]
        public float jumpHeight = 4f;
        [Tooltip("Add a jump delay.")]
        ...
        internal bool
            ...
            isInAirborne,
            isTurningOnSpot,
            jumpingCooldown; // [ODYSSEY] Jump CD fix
        ...
        {
            if (!isJumping && !jumpingCooldown) return;

            if (isJumping) jumpCounter -= Time.deltaTime;
            finalJumpCounter -= Time.deltaTime;
            ...
            if (finalJumpCounter <= 0) // [ODYSSEY] Fix jump cooldown 
            {
                finalJumpCounter = 0;
