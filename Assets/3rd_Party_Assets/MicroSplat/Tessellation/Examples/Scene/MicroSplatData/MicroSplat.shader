...
// post vertex
   ...
   #define MAX_TESSELLATION_FACTORS 64.0
      ...
      MicroSplatLayer SurfImpl(Input i, float3 worldNormalVertex)
      {
         ...
         #if _MSRENDERLOOP_SURFACESHADER && !_DEBUG_USE_TOPOLOGY &&!_TERRAINBLENDABLESHADER && !_MICROMESH && !_MICROMESHTERRAIN && !_MICROPOLARISMESH &&!_MICRODIGGERMESH && !_MICROVERTEXMESH && defined(UNITY_INSTANCING_ENABLED) && !defined(SHADER_API_D3D11_9X)
            float3 t2w0 = WorldNormalVector(i, float3(1,0,0));
            float3 t2w1 = WorldNormalVector(i, float3(0,1,0));
            float3 t2w2 = WorldNormalVector(i, float3(0,0,1));
            float3x3 t2w = float3x3(t2w0, t2w1, t2w2);
...
ENDCG
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Tessellation_Base1812756706"
