...
Shader "MicroSplat/Example_Streams" {
   Properties {
      ...
      _LavaEmissiveMult("Emission Multiplier", Float) = 2
...
[HideInInspector] _DoubleSidedConstants("Vector4", Vector) = (1,1,-1,0)
   ...
   SubShader {
      Tags{"RenderPipeline"="HDRenderPipeline" "RenderType" = "HDLitShader" "Queue" = "Geometry+100" "SplatCount" = "8"}
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
         ...
         #if _LAVA
            UNITY_DECLARE_TEX2D_NOSAMPLER(_LavaDiffuse);
         #endif

         #if _RAINDROPS
            UNITY_DECLARE_TEX2D_NOSAMPLER(_RainDropTexture);
         #endif
         ...
         {
            half4 ripple = UNITY_SAMPLE_TEX2D_SAMPLER(_RainDropTexture, _Diffuse, uv);
         ...
         half4 SampleLava(float2 uv, float2 dx, float2 dy)
         {
               ...
               half4 S1 = MICROSPLAT_SAMPLE_TEX2D_SAMPLER_GRAD(_LavaDiffuse, _Diffuse, uv1, dx, dy);
               half4 S2 = MICROSPLAT_SAMPLE_TEX2D_SAMPLER_GRAD(_LavaDiffuse, _Diffuse, uv2, dx, dy);
               half4 S3 = MICROSPLAT_SAMPLE_TEX2D_SAMPLER_GRAD(_LavaDiffuse, _Diffuse, uv3, dx, dy);
               COUNTSAMPLE
               COUNTSAMPLE
               COUNTSAMPLE
            ...
            #else
               COUNTSAMPLE
               return MICROSPLAT_SAMPLE_TEX2D_SAMPLER_GRAD(_LavaDiffuse, _Diffuse, uv, dx, dy);
            #endif
         }
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Streams_Base-1340454434"
